    ...
    <div className="min-h-screen bg-linear-to-br from-slate-50 to-slate-100 dark:from-slate-950 dark:to-slate-900 flex flex-col">
      <header className="px-6 py-5 flex items-center justify-between max-w-6xl mx-auto w-full">
        ...
        <Link
          ...
          className="hidden sm:inline-flex items-center gap-1.5 text-xs text-muted-foreground border rounded-full px-3 py-1.5 bg-background hover:text-foreground transition-colors"
        >
          Open storefront
      ...
      <main className="flex-1 px-4 pb-16 max-w-6xl mx-auto w-full">
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {PERSONAS.map(p => (
            <Link
              key={p.role}
              to={p.to as any}
              params={p.role === 'Customer' ? { slug: DEFAULT_1TO99_STOREFRONT_SLUG } : undefined}
        ...
        <p className="mt-10 text-center text-xs text-muted-foreground">
          ...
          <code className="font-mono bg-background border rounded px-1.5 py-0.5 text-[11px]">{DEFAULT_1TO99_STOREFRONT_SLUG}</code>
        </p>
      </main>
    </div>
  )
